...
void main(void){
    vec4 sample_tex = edge_detection(inputImageTexture, textureCoordinate);
    float gray = sample_tex.x * 0.3 + sample_tex.y * 0.59 + sample_tex.z * 0.11;
    if (gray > 0.01) {
        sample_tex = vec4(0.0, 0.0, 0.0, 1.0);
    } else {
        sample_tex = vec4(1.0);
    }
